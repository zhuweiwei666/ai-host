  ...
  return (
    <div className={`group glass rounded-2xl p-6 flex flex-col transition-all duration-300 card-hover relative overflow-hidden ${
      ...
      <div className="relative z-10 flex flex-col">
        ...
        {/* 累计互动次数 */}
        {(agent.stats?.totalInteractions ?? 0) > 0 && (
          <div className="mb-3 flex items-center justify-center gap-1.5 text-xs text-gray-500">
            ...
            <span>{(agent.stats?.totalInteractions ?? 0).toLocaleString()} 次互动</span>
          </div>
        )}

        {/* Story Mode Button */}
        <button
          onClick={() => navigate(`/story/${agent._id}`)}
          className="w-full flex justify-center items-center gap-2 px-5 py-3 text-sm font-semibold rounded-xl text-white bg-gradient-to-r from-pink-600 via-purple-600 to-indigo-600 hover:from-pink-700 hover:via-purple-700 hover:to-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-pink-500 transition-all duration-300 shadow-lg hover:shadow-xl hover:shadow-pink-500/30 hover:scale-[1.02] active:scale-[0.98] mt-auto"
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
